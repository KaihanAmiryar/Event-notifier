...
// @ts-nocheck
	...
	let data = {};
	let crime_Id = $page.params.id;
	onMount(async () => {
		data = await axios
			.post('http://localhost:3000/api/crime/show', { crime_Id })
	...
	async function handleSubmit() {
		await axios
			.post('http://localhost:3000/api/crime/update', { crime_Id, ...data})
...
<div class="details" style="direction: rtl;">
	<div class="event">
		<form class="form" on:submit|preventDefault={handleSubmit}>
			...
			<div class="form-group">
				<label>ولایت</label>
				<input type="text" bind:value={data.province} class="form-control m-2" />
			</div>
			<div class="form-group">
				<label>ولسولی</label>
				<input type="text" bind:value={data.district} class="form-control m-2" />
			</div>
			<div class="form-group">
				<label>توضیحات</label>
				<textarea bind:value={data.description} class="form-control m-2" />
			</div>
			<div class="form-group m-2">
				<label>وضعیت</label>
				<select bind:value={data.status} class="form-control">
